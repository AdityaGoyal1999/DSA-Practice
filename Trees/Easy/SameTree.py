"""
Check if the 2 trees are the same
"""
...
def sameTree(p, q):
    if not p and not q:
        ...
        if not sameTree(p.left, q.right) or not sameTree(p.right, q.left):
            return False
...
print(sameTree(a.left, a.right) == False)
print(sameTree(b.left, b.right) == True)